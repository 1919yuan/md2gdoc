...
function importMarkdown(markdown) {
  ...
  PropertiesService.getScriptProperties().setProperty('listNestingLevel', JSON.stringify(-1));
  PropertiesService.getScriptProperties().setProperty('prevListId', JSON.stringify([]));
  PropertiesService.getScriptProperties().setProperty('glyphType', JSON.stringify([]));
  PropertiesService.getScriptProperties().setProperty('inBlockquote', JSON.stringify(false));
...
function h1(e) {
  ...
  var h = body.appendParagraph(e.getText());
...
function h4(e) {
  var body = DocumentApp.getActiveDocument().getBody();
  var h = body.appendParagraph(e.getText());
  h.setHeading(DocumentApp.ParagraphHeading.HEADING4);
}
...
function h6(e) {
  var body = DocumentApp.getActiveDocument().getBody();
  var h = body.appendParagraph(e.getText());
  h.setHeading(DocumentApp.ParagraphHeading.HEADING6);
}

function p(e) {
  var level = JSON.parse(PropertiesService.getScriptProperties().getProperty('listNestingLevel'));
  var blockquote = JSON.parse(PropertiesService.getScriptProperties().getProperty('inBlockquote'));
  if (level != -1) {
    processDescendents(e.getDescendants(), e);
  } else if (blockquote) {
    var cell = getLastTableCell();
    var paragraph = cell.appendParagraph("a");
    paragraph = paragraph.clear();
    processDescendents(e.getDescendants(), e);
  } else {
    var body = DocumentApp.getActiveDocument().getBody();
    var paragraph = body.appendParagraph("a");
    paragraph = paragraph.clear();
    processDescendents(e.getDescendants(), e);
  }
}
...
function blockquote(e) {
  ...
  var body = DocumentApp.getActiveDocument().getBody();
  var inBlockquote = JSON.parse(PropertiesService.getScriptProperties().getProperty('inBlockquote'));
  if (inBlockquote) {
    throw new Error("Nested levels of Blockquote is not supported.");
  }
  PropertiesService.getScriptProperties().setProperty('inBlockquote', JSON.stringify(true));
  processDescendents(e.getDescendants(), e);
  PropertiesService.getScriptProperties().setProperty('inBlockquote', JSON.stringify(false));
}
...
function ul(e) {
  var level = JSON.parse(PropertiesService.getScriptProperties().getProperty('listNestingLevel')) + 1;
  PropertiesService.getScriptProperties().setProperty('listNestingLevel', JSON.stringify(level));
  var prevListIds = JSON.parse(PropertiesService.getScriptProperties().getProperty('prevListId'));
  prevListIds.push(null);
  PropertiesService.getScriptProperties().setProperty('prevListId', JSON.stringify(prevListIds));
  var glyphTypes = JSON.parse(PropertiesService.getScriptProperties().getProperty('glyphType'));
  glyphTypes.push(DocumentApp.GlyphType.BULLET);
  PropertiesService.getScriptProperties().setProperty('glyphType', JSON.stringify(glyphTypes));
...
function close_ul(e) {
  var level = JSON.parse(PropertiesService.getScriptProperties().getProperty('listNestingLevel'));
  PropertiesService.getScriptProperties().setProperty('listNestingLevel', JSON.stringify(level - 1));
  var prevListIds = JSON.parse(PropertiesService.getScriptProperties().getProperty('prevListId'));
  prevListIds.pop();
  PropertiesService.getScriptProperties().setProperty('prevListId', JSON.stringify(prevListIds));
  var glyphTypes = JSON.parse(PropertiesService.getScriptProperties().getProperty('glyphType'));
...
function li(e) {
  var body = DocumentApp.getActiveDocument().getBody();
  var level = JSON.parse(PropertiesService.getScriptProperties().getProperty('listNestingLevel'));
  var glyphType = JSON.parse(PropertiesService.getScriptProperties().getProperty('glyphType'))[level];
  var prevListId = JSON.parse(PropertiesService.getScriptProperties().getProperty('prevListId'))[level];
  var item = body.appendListItem("a");
  item = item.clear();
  item = item.setNestingLevel(level).setGlyphType(DocumentApp.GlyphType[glyphType]);
  if (prevListId) {
...
function getLastListItemById(id) {
  ...
  for (var i = 0; i < doc.getNumChildren(); i++) {
    ...
    if (child.getType() == DocumentApp.ElementType.LIST_ITEM && child.getListId() == id) {
      ret = child;
    }
  }
  return ret;
